...
//	}
	...
	@Override
	//pour les CTs coeurs de métiers => lecture des step liés à une exigence ...
...
//				.from(REQUIREMENT_VERSION_COVERAGE)
//				.where(REQUIREMENT_VERSION_COVERAGE.VERIFIED_REQ_VERSION_ID.in(reqId))
//				.fetchInto(ReqTestCaseBinding.class);			
//
//	}
...
//		order by tcs.step_order ASC
		   return    dsl.select(TEST_CASE_STEPS.STEP_ID)
					.from(TEST_CASE)
					.innerJoin(TEST_CASE_LIBRARY_NODE).on(TEST_CASE_LIBRARY_NODE.TCLN_ID.eq(TEST_CASE.TCLN_ID))
					.innerJoin(TEST_CASE_STEPS).on(TEST_CASE_STEPS.TEST_CASE_ID.eq(TEST_CASE.TCLN_ID))
					.where(TEST_CASE.TCLN_ID.in(tc_id))
